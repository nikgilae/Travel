...
- 2026-08-26 — **Мини-фикс промпта `generate_trip`** (не часть
  пронумерованных итераций, реакция на прод-находку). Реальный кейс: интересы
  `food, night, relaxed` дали `main_pois` дня из собора/музея/башни (0 из 3 —
  food/night), хотя retrieval передал AI верно ранжированные food/night
  кандидаты (диагностика: прямой вызов `get_relevant_for_trip` внутри прод-
  контейнера вернул топ-15 сплошь food/night; город имел 207 POI при
  `MAX_POIS_FOR_AI=100` — фильтрация реально произошла, не no-op). Причина —
  в шаге генерации, не в retrieval: модель предпочитает "must-see"
  достопримечательности вне зависимости от интересов. Промпт (normal и fast,
  `backend/app/services/ai.py`) обновлён: не запрет достопримечательностей
  (это ценная часть тревел-опыта), а лимит — не более одной на день, если она
  не соответствует интересам, остальные main_pois дня обязаны соответствовать.
  Закоммичено (`6b954ca`), задеплоить решено вместе с Итерацией 4.
- 2026-08-26 — **Итерация 4 выполнена, оценка — NO-GO для текущей реализации.**
  `backend/eval/rag/iteration4-report.md`. Обогащение (Google editorial_summary
  → саммари отзывов → fallback по категории, `scripts/enrich_poi_descriptions.py`)
  отработало технически верно на всём dev-корпусе (346/346 POI: 54
  `google_editorial`, 281 `reviews_summary`, 11 `category_fallback`), текст
  качественный, без явных признаков галлюцинаций. **Но retrieval-качество
  измеримо просело**: recall@40 (production_shape) 0.509→0.373, recall@100
  0.799→0.632, MRR 0.813→0.722 — регрессия равномерная по всем 8 вариациям
  запроса (4 тега × tag-only/with-notes), не один проблемный тег. Всё ещё
  кратно лучше random-baseline (3.2× против было 4.1×), но хуже того, что уже
  было в проде. Причина (правдоподобная, не догадка): `ground_truth.json`
  размечен по совпадению Google Place types в тексте — старый `description`
  буквально содержал эти типы, новый `build_poi_text` **заменял** его на
  `name + ai_description` целиком, вымывая лексическое совпадение с тегами,
  на котором держался recall. Цель, ради которой затевалась итерация
  (`relaxed`/`active`) — качественно неоднозначный результат, не явное
  улучшение. **Код и колонки (`ai_description`/`ai_description_source`)
  оставлены в базе** — обогащение само по себе не вредно и полезно как
  more live текст для карточки места (не связано с retrieval), но
  `build_poi_text` не должен безусловно заменять текст для эмбеддинга.
  Правдоподобный фикс на будущее — дополнять текст (`name + description +
  ai_description`), а не заменять — отдельный, ещё не сделанный эксперимент.
  **Прод не трогается: RAG_POI_RETRIEVAL_ENABLED и так уже в проде на
  прежнем (не обогащённом) тексте — регрессия эта итерация внесла бы, только
  если прод-корпус тоже обогатить и пересчитать эмбеддинги, чего сделано не
  было.**
  **Тем же днём — Эксперимент 2, "дополнять, не заменять":** `build_poi_text`
  изменён на `name + description + information + ai_description` (было —
  замена на `name + ai_description`). Регрессия сжалась с ~27% до ~6-7%
  относительных (production_shape recall@40 0.509→**0.477**, recall@100
  0.799→**0.747**, MRR 0.813→**0.713**) — почти всё лексическое совпадение
  с ground truth восстановлено, часть запросов вернулась к исходным цифрам
  байт-в-байт. `relaxed`/`active` качественно вернулись к исходному тесному
  кластеру (spa/lounge, батутный парк/аквапарк) — ни явного улучшения, ни
  ухудшения. Код обновлён и закоммичен (`c547c93`), полные данные —
  `backend/eval/rag/iteration4-report.md`.
  **Итог: приемлемый компромисс, не чистый go.** Небольшая (не кратная)
  просадка retrieval ради более насыщенного текста для будущего отображения
  в UI (второй мотив итерации, независимый от retrieval) — но исходная цель
  измерения (сигнал для `relaxed`/`active`) так и не подтверждена численно.
  Прод-запуск обогащения на боевых 3632 POI — отдельное явное решение, не
  автоматическое продолжение этой записи.
- 2026-08-26 — **Код мини-фикса и Итерации 4 задеплоен на прод (миграция +
  api), реальное обогащение POI — отложено.** `git push` (`ffe23f6..f9dd793`),
  на сервере: `pg_dump` бэкап, `git pull`, `DOCKER_BUILDKIT=0 docker compose
  build api`, `docker compose up -d api` — миграция `afbe2bbd84e2 →
  c752eabffe3a` (колонки `ai_description`/`ai_description_source`) прошла
  чисто через `entrypoint.sh`, подтверждено `\d pois`. Мини-фикс промпта живой
  на проде с этого момента.
  **Перед реальным запуском `scripts.enrich_poi_descriptions` на 3632 боевых
  POI проверена актуальная цена** (Google Maps Platform pricing, август 2026):
  `editorial_summary` и `reviews` в Place Details оба триггерят самый дорогой
  тариф **Enterprise + Atmosphere** — 1000 запросов/мес бесплатно, дальше
  $25.00/1000 (первый платный порог). На 3632 POI это ≈$66 разово. **Решение
  пользователя: отложить** до определения, какие страны/города вообще
  остаются в проде (нет смысла платить за обогащение POI городов, которые
  потом могут быть выпилены) — и потому что сейчас нет на это бюджета.
  Возврат к этому шагу — отдельное явное решение позже, не по умолчанию.
