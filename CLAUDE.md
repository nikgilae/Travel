...
В конце сессии дописывай появившиеся задачи в файл ~/tasks/inbox.md.

Правила:
- Только дописывай в конец файла. Никогда не редактируй и не
  удаляй существующие строки. Файла нет — создай.
- Формат строки: - [проект] действие — Nм
  Пример: - [tourism-marketing] написать текст первого
  сообщения для волны №1 — 25м
- Одна строка = одно действие. Оценка в минутах обязательна.
- Потолок блока — 25 минут. Задача больше — режь на несколько
  строк по 25м. Не знаешь оценку — округляй вверх до 25м.
- Формулируй как физическое действие, а не как тему
  («открыть форму и заполнить первый экран», не «резюме»).
- Перед первой строкой сессии ставь заголовок с сегодняшней
  датой в формате ## 2026-08-06, если его ещё нет в файле.
- Не ставь приоритеты, дедлайны, даты выполнения, порядок.
  Не сортируй. Не помечай задачи как важные или срочные.
- Не предлагай мне взяться за задачу сейчас. Инбокс — склад,
  не расписание.
- В инбокс идёт только то, что делаю руками я. Не пиши туда
  то, что ты можешь сделать в этой же сессии сам.
