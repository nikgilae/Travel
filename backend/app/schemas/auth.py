...
class ClaimRequest(RegisterRequest):
    """
    Схема запроса на превращение гостевого аккаунта в настоящий.

    Поля и правила те же, что у регистрации (наследуется целиком, чтобы
    требования к паролю не разъезжались между двумя формами входа).
    Отличие только в том, что аккаунт уже существует и берётся из токена.
    """
