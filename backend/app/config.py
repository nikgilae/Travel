...
class Settings(BaseSettings):
    ...
    # Наполнять город местами из памяти AI-модели, когда Google Maps ничего не
    # дал. Включено вынужденно: с 2026-09-09 у проекта в Google Cloud выключен
    # биллинг, Places API отвечает REQUEST_DENIED на каждый запрос, и любой
    # город, которого ещё нет в справочнике, остаётся пустым навсегда.
    #
    # ВЫКЛЮЧИТЬ (AI_POI_FALLBACK_ENABLED=false в .env), как только биллинг
    # включён обратно: места от модели не проверены Google, у них
    # приблизительные координаты и нет google_place_id. Они помечены в базе
    # как source='ai_fallback' — этим же признаком их потом можно вычистить,
    # чтобы Google собрал город заново.
    AI_POI_FALLBACK_ENABLED: bool = True

    # Сколько мест просить у модели на город. Двадцать пять — примерно то же,
    # что даёт Google на средний город после дедупликации, и столько модель
    # ещё выдаёт одним ответом без обрезки.
    AI_POI_FALLBACK_COUNT: int = 25
